...
a1 = 0.2073; %trans_std in mm
b1 = 0; %trans_mean

a2 =0.00425;%angle_std in deg
b2=0.7760; %angle_mean

tolerance = 0;

%% Read the peg and hole STL Files
[Fp, Vp, Cp] = rndread('STL_Files/Peg.STL');
Vp = Vp';
Vp = [Vp(1,:); Vp(2,:); Vp(3,:); ones(1,length(Vp))];
trans = tl(0.5, 0.5, 11.3);%*rot;
Vp=trans*Vp; 

Vp = [Vp(1,:); Vp(2,:); Vp(3,:);];
Surface_peg = delaunayTriangulation(Vp(1,:)',Vp(2,:)',Vp(3,:)');

[Fh, Vh, Ch] = rndread('STL_Files/Hole.STL');%'Hole_Mouth.STL');
Vh=Vh';
Surface_hole = delaunayTriangulation(Vh(1,:)',Vh(2,:)',Vh(3,:)');
list=Surface_hole.ConnectivityList;
rows_to_remove = any(list==240, 2);
list(rows_to_remove,:) = [];
rows_to_remove = any(list==186, 2);
list(rows_to_remove,:) = [];

figure;
Surface_hole2=triangulation(list,Surface_hole.Points);
tetramesh(Surface_hole2, 'FaceColor', 'red');
%hold on;
figure;
tetramesh(Surface_peg, 'FaceColor', 'green');
alpha(0.4)
clear axes;

Original_Copy_of_Vp=Vp';


%% Compute Success Probability

% Generate error numbers
N=30; % Number of Trials
Count=0;
%rand = [a1.*randn(N,1) + b1, a1.*randn(N,1) + b1, a1.*randn(N,1) + b1, a2.*randn(N,1) + b2, a2.*randn(N,1) + b2,a2.*randn(N,1) + b2];
rand = [a1.*randn(N,1) + b1, 0.*randn(N,1), a1.*randn(N,1) + b1, 0.*randn(N,1) , a2.*randn(N,1) + b2,0.*randn(N,1)];
%load matlab
%figure;
for trial=1:N
    Vp = Original_Copy_of_Vp';
    Vp = [Vp(1,:); Vp(2,:); Vp(3,:); ones(1,length(Vp))];
    
    %Add noise to the peg
    rot = (rz(rand(trial, 4))*ry(rand(trial, 5))*rx(rand(trial, 6))); %Euler angles (rz(alpha)*ry(beta)*rx(gamma))*peg; %Euler angles
    trans = tl(rand(trial, 1),rand(trial, 2),rand(trial, 3));
    trans(1:3, 1:3) = rot(1:3, 1:3);
    %trans = tl(0.5, 0.5, 0);%*rot;
    Vp=trans*Vp; 
...
%     alpha(0.4)
   ...
   [~, SurfAB] = SurfaceIntersection(Surface_hole2,Surface_peg)
   
   if(isempty(SurfAB.vertices)==1)
       Count = Count+1;
   end
end
...
function T = tl(x,y,z)
% TRANSLATION ALONG THE Roll, Pitch, AND Yaw AXES
% T = tl(roll,pitch,yaw)
...
% Rx = rx(THETA)
% This is the homogeneous transformation for rotation about the Roll-axis.
% NOTE:  The angle THETA must be in DEGREES.
THETA = THETA*pi/180;  % converting THETA in radians.
c = cos(THETA);
s = sin(THETA);
...
% Ry = ry(THETA)
% This is the homogeneous transformation for rotation about the Pitch-axis.
% NOTE: The angle THETA must be in DEGREES.
THETA = THETA*pi/180;  % converting THETA is in radians.
c = cos(THETA);
s = sin(THETA);
...
% Rz = rz(THETA)
% This is the homogeneous transformation for rotation about the Yaw-axis.
% NOTE:  The angle THETA must be in DEGREES.
THETA = THETA*pi/180;  % converting THETA is in radians.
c = cos(THETA);
s = sin(THETA);
